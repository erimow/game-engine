...
void Slider_init(Slider *slider, SDL_FRect *lever,  SDL_Color leverColor,SDL_FRect* bar, SDL_Color barColor, int defaultVal, int minVal, int maxVal, bool isVert){
  ...
  if (slider->value<minVal)
    slider->value = minVal;
  if (isVert)
  slider->sliderLever.y = (slider->sliderBar.y)+((slider->sliderBar.h-slider->sliderLever.h)-(float)(((slider->max-slider->min)-(slider->value))/slider->stepLength));
  else
  slider->sliderLever.x = slider->sliderBar.x+((slider->sliderBar.w-slider->sliderLever.w)-(float)(((slider->max-slider->min)-(slider->value))/slider->stepLength));
}

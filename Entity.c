...
void Entity_init(Entity *entity, float xPos, float yPos, float spriteWidth,
  ...
  entity->clip = (SDL_FRect *)malloc(sizeof(SDL_Rect) * spriteAmount);
  if (entity->clip == NULL)
    printf("clip not properly malloced!\n");
  else
    for (int i = 0; i<spriteAmount; i++){
      entity->clip[i].x = TILE_SIZE*i;
      entity->clip[i].y = i/(TILE_SIZE);
      entity->clip[i].w = TILE_SIZE;
      entity->clip[i].h = TILE_SIZE;
    }
...
void Entity_render(Entity *entity, SDL_Renderer *renderer, SDL_FRect *clip,
                   Uint8 clipToRender, SDL_FPoint *center,
                   SDL_FlipMode flip, Camera *camera, float depthZ) {
  SDL_FRect pos;
  if (camera != NULL)
